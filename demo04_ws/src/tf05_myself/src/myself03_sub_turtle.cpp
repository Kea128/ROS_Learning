...
int main(int argc, char* argv[]) {
  ...
  while (ros::ok()) {
    try {
      ...
      _1To2 =
          buffer.lookupTransform("body/pose_02", "body/pose_01", ros::Time(0));

      ROS_INFO("pose_01相对于pose_02的关系:位移(%.2f,%.2f,%.2f)",
               _1To2.transform.translation.x, _1To2.transform.translation.y,
               _1To2.transform.translation.z);

      float distance = sqrt(pow(_1To2.transform.translation.x, 2) +
                            pow(_1To2.transform.translation.y, 2));
      float yawError =
          atan2(_1To2.transform.translation.y, _1To2.transform.translation.x);
